...
export default function LostItems() {
  ...
  // Filter items by type ('Lost') and search query
  ...
  return (
    <div className="w-full flex flex-col items-center min-h-[80vh] pb-16">
      {/* Header Banner */}
      <section className="w-full bg-gradient-to-r from-rose-950 via-slate-900 to-rose-900 text-white py-12 px-4 sm:px-6 lg:px-8 border-b border-rose-900/30">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div>
            ...
            <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-white m-0">
              Community Lost Items
            </h1>
            <p className="text-rose-100/80 text-sm sm:text-base mt-2 max-w-xl">
              {user?.nickname ? `Welcome back, ${user.nickname}! ` : ''}
              Have you seen any of these belongings? Help the owner by reporting your findings.
            </p>
          </div>

          <Link
            to="/postitem"
            className="inline-flex items-center gap-2 px-5 py-3 rounded-xl font-semibold text-white bg-rose-600 hover:bg-rose-500 shadow-lg shadow-rose-600/30 transition-all text-sm shrink-0"
          >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-8">
          <div className="relative w-full sm:w-96">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-200 bg-white text-sm focus:outline-none focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20 shadow-sm transition-all"
            />
          </div>

          <div className="text-xs font-semibold text-slate-500 self-end sm:self-center">
            Showing <span className="text-slate-900 font-bold">{filteredItems.length}</span> active report(s)
          </div>
        </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-3xl border border-slate-200/80 shadow-sm p-8">
            ...
            <p className="text-sm text-slate-500 max-w-sm mb-6">
              {searchQuery
                ...
                : 'There are currently no active lost item reports in the community.'}
            </p>
            <Link
              to="/postitem"
              className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-semibold text-white bg-rose-600 hover:bg-rose-500 shadow-md text-sm transition-all"
            >
              <HiPlusCircle className="w-5 h-5" />
                ...
                >
                  <div>
                    ...
                    <div className="relative aspect-[16/10] bg-slate-100 overflow-hidden">
                      ...
                      {/* Status Tag */}
                      <div className="absolute top-3 left-3 bg-rose-600/95 backdrop-blur-md text-white px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider shadow-sm flex items-center gap-1.5">
                      ...
                      <div className="absolute bottom-3 left-3 right-3 bg-slate-900/80 backdrop-blur-md text-white px-3 py-1 rounded-lg text-xs font-medium truncate flex items-center gap-1.5">
                        <HiMapPin className="w-3.5 h-3.5 text-rose-400 shrink-0" />
                    ...
                    {/* Card Content */}
                    <div className="p-5">
                      <h3 className="text-lg font-bold text-slate-900 group-hover:text-rose-600 transition-colors truncate mb-2">
                        {item.name}
                      </h3>
                  ...
                  <div className="p-5 pt-0">
                    <Link
                      to={`/${encodeURIComponent(item.name)}?cid=${item._id}&type=Lost/${isOwner ? 'true' : 'false'}`}
                      className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-semibold text-sm text-slate-700 bg-slate-100 hover:bg-rose-50 hover:text-rose-700 transition-colors"
                    >
